...
export default context => ({
  ...
  getters: {
    ...
    avatarUrl(state){
      if(state && state.info && state.info.avatarUrl)
        return state.info.avatarUrl.replace('ctxPath','js') || '/';
      else return '/'
    }
  },
  actions: {
    ...
    async login ({ commit, dispatch }, { username = '', password = '', to = '/' } = {}) {
      try {
       ...
       utils.cookies.set('uuid', data.user.id)
       utils.cookies.set('token', data.sessionid)
        // 设置用户已经登陆
        commit('isLoggedSet', true)
        // 设置 vuex 用户信息
        await dispatch('d2admin/user/set', data.user, { root: true })
        // 加载权限
